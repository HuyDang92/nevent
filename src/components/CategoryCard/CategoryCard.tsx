...
const CategoryCard = ({ data }: CategoryCardProps) => {
  return (
    <div className={`group relative z-10 w-full overflow-hidden rounded-xl transition `}>
      <Link to={'/'}>
        <div className="absolute z-30 h-52 w-full rounded-xl bg-cs_dark opacity-50"></div>
        <img
          src={data?.image?.url}
          alt="cate"
          className="h-52 w-full group-hover:scale-110 rounded-xl object-cover transition-all"
        />
