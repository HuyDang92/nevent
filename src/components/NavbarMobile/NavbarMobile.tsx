  ...
  return (
    <>
      ...
      >
        ...
        {auth.loggedIn && auth.currentUser?.role?.name === 'business' && (
          <motion.button whileTap={{ scale: 0.9 }}>
            <NavLink to={'/organization/organization-profile'} className={`flex items-center justify-between`}>
              <Icon
                name={location.pathname === '/organization/organization-profile' ? 'person' : 'person-outline'}
                className="text-2xl text-cs_semi_green"
              />
            </NavLink>
          </motion.button>
