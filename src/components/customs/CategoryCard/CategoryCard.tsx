...
const CategoryCard = ({ id, image, link, name, className }: CategoryCardProps) => {
  return (
    <div className={`relative w-full z-10 overflow-hidden rounded-xl transition  ${className}`} key={id}>
      <Link to={link}>
        <div className="absolute z-30 h-52 w-full rounded-xl bg-cs_purple opacity-50"></div>
        <img src={image}  alt="cate" className="h-52 w-full hover:scale-110 rounded-xl object-cover" />
