...
const ProductCard = ({ id, place, image, link, name, date, category, className }: ProductCardProps) => {
  return (
    <div className={`relative h-64 w-full cursor-pointer rounded-full shadow-lg ${className}`} key={id}>
      <Link to={link}>
        <img src={image} alt="image" className="h-44 w-full rounded-xl object-cover" />
        <div className=" absolute bottom-0 z-10 mt-5 rounded-xl rounded-t-lg bg-white p-3 text-left font-bold text-cs_dark shadow-lg">
