...
const TicketProfile: React.FC<IProps> = ({ data, passTicket }) => {
  ...
  return (
    <div className={`${data?.myTickets?.length > 5 ? ' mx-8' : 'mx-6'} relative my-5 sm:mx-16`}>
          ...
          return (
            <SwiperSlide key={index} className="relative rounded-2xl">
              <div ref={qrCodeRefs.current[index]}>
                <div className="round-2xl absolute z-10 h-full w-full rounded-2xl bg-cs_dark opacity-50 transition-all group-hover:scale-110"></div>
                <img
                  src={data?.event?.banner[0]?.url}
                  alt=""
                  className="h-[480px] w-full rounded-xl object-cover xl:h-[310px]"
                />
                <div className="absolute left-0 top-0 z-10 flex w-full justify-between p-4 text-cs_light">
                  ...
                  <div className="absolute -bottom-[110%] right-1/2 h-56 w-56 translate-x-1/2 xl:-bottom-[6.5rem] xl:right-5 xl:mt-12 xl:translate-x-0">
                    <div className="flex justify-center rounded-xl border bg-cs_light py-2 text-cs_dark shadow-border-light">
                      <div>
                        ...
                        <QRCode className="h-52 w-52 bg-cs_light p-2 xl:h-52 xl:w-52" id="qrcode" value={item?.qr} />
                      </div>
                    </div>
                  </div>
                </div>
